...
from src.models.cra import CRA
...
parser.add_argument('--model', type=str, default='vae', metavar='M', help='Model selection')
...
if args.model.lower() == 'vae':
    ...
    model = CRA((200, 100, 10), input_dim=n_features, dropout=args.dropout, num_ra=2)
else:
    model = DAE((200, 100, 10), input_dim=n_features, dropout=args.dropout)
...
# Heatmap
for i, mod in enumerate(('cpm', 'steps')):
    vis.heatmap(range(args.epochs), test_batch[:, :, i], opts=dict(colormap='Electric', title=mod))
    vis.heatmap(range(args.epochs), recon_batch[:, :, i], opts=dict(colormap='Electric', title=mod))

# Errors
vis.line(np.stack((train_loss[1:], test_loss[1:]), axis=1), opts=dict(legend=['train', 'test']))